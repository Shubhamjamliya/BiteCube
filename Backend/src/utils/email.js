...
export async function sendAdminResetOtpEmail(to, otp) {
    ...
    const subject = 'Your password reset code – Bitecube Admin';
    const html = `
<!DOCTYPE html>
<html>
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 480px; margin: 0 auto; padding: 20px;">
  ...
  <p style="color: #999; font-size: 12px;">Bitecube Admin</p>
</body>
</html>`;
    ...
    try {
        await trans.sendMail({
            from: typeof from === 'string' && from.includes('<') ? from : `Bitecube <${from}>`,
            to,
            subject,
            text,
            html
...
export async function sendRestaurantOnboardingEmail(to, restaurantName, pdfUrl) {
    ...
    const from = config.emailFrom || config.emailUser;
    const subject = `Welcome to Bitecube, ${restaurantName}!`;
...
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  ...
  <p>Your restaurant has been successfully onboarded to Bitecube.</p>
