...
export default function AddEditSubcategoryModal({
  ...
  useEffect(() => {
    if (subcategoryToEdit) {
      const parentCatId = subcategoryToEdit.categoryId?._id || subcategoryToEdit.categoryId || '';
      setFormData({
        ...
        sortOrder: subcategoryToEdit.sortOrder ?? 0,
        isActive: subcategoryToEdit.isActive !== undefined ? subcategoryToEdit.isActive : true,
      });
      setUploadTab(subcategoryToEdit.image ? 'url' : 'file');
    } else {
      setFormData({
        categoryId: categoriesList.length > 0 ? categoriesList[0]._id : '',
        name: '',
        slug: '',
        description: '',
        image: '',
    ...
    setErrorMsg('');
  }, [subcategoryToEdit, isOpen, categoriesList]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrorMsg(err.response?.data?.message || err.message || 'Failed to save subcategory.');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          ...
          <div className="flex items-center justify-end gap-3 pt-4 border-t border-slate-200">
            ...
            >
              ...
              {subcategoryToEdit ? 'Save Changes' : 'Create Subcategory'}
            </button>
          </div>
        </form>
      </div>
