...
export default function DeleteSubcategoryConfirmModal({ isOpen, onClose, onConfirm, subcategoryToDelete }) {
  const [deleting, setDeleting] = useState(false);

  if (!isOpen || !subcategoryToDelete) return null;

  const handleConfirm = async () => {
    try {
      setDeleting(true);
      await onConfirm(subcategoryToDelete._id);
      onClose();
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/40 backdrop-blur-xs p-4 overflow-y-auto">
      <div className="relative w-full max-w-md rounded-2xl bg-white shadow-xl overflow-hidden border border-slate-200 animate-in fade-in zoom-in-95 duration-150 p-6 space-y-4">
        ...
        <p className="text-sm text-slate-600 bg-slate-50 p-3 rounded-xl border border-slate-200">
          Are you sure you want to delete <span className="font-bold text-slate-900">"{subcategoryToDelete.name}"</span>?
        </p>
