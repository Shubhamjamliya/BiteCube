...
import { Pencil, Trash2, Image as ImageIcon, ChevronLeft, ChevronRight, Loader2 } from 'lucide-react';
  ...
  if (loading) {
    return (
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white p-16 text-center shadow-sm">
        ...
        <p className="mt-2 text-sm font-medium text-slate-500">Loading categories...</p>
      </div>
    );
  }
    ...
    return (
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white p-16 text-center shadow-sm">
        ...
        <p className="mt-1 text-sm text-slate-500 max-w-sm mx-auto">
          Try a different search term or click "Add Category" to create one.
        </p>
      </div>
    );
  }
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-slate-100">
            {categories.map((cat) => (
              <tr key={cat._id} className="align-middle hover:bg-slate-50/80 transition-colors">
                ...
                <td className="px-5 py-4">
                  <div className="flex items-center gap-3">
                    <div className="h-11 w-11 overflow-hidden rounded-2xl bg-slate-100 flex-shrink-0 flex items-center justify-center border border-slate-200">
                      ...
                      <div className={`h-full w-full flex items-center justify-center text-sm font-bold text-slate-500 ${cat.image ? 'hidden' : ''}`}>
                        {String(cat?.name || "C").slice(0, 1).toUpperCase()}
                      </div>
                    </div>
                    <div className="min-w-0">
                      <p className="truncate text-base font-semibold leading-6 text-slate-900">{cat.name}</p>
                    </div>
                  </div>
                </td>
                ...
                <td className="px-5 py-4 text-center">
                  <button
                    ...
                    className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors cursor-pointer ${
                      cat.isActive ? "bg-blue-600" : "bg-slate-300"
                    }`}
